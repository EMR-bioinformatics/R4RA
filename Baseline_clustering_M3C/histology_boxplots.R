histoboxplot <- function(meta,variable,toadd,xlab='no',fill=10,theme='bw',colours=FALSE,
  ...
  if (theme == 'bw' && rotatex == FALSE){ # use black and white theme
    p <- ggplot(meta2, aes_string(x = variable, y = toadd)) +
      geom_boxplot(aes_string(fill = variable), lwd=0.5,colour = "black", outlier.shape = NA, coef=0) +
      #geom_point(aes_string(fill = variable), size = 2, shape = 21) +
      theme_bw() +
      labs(title = toadd) +
