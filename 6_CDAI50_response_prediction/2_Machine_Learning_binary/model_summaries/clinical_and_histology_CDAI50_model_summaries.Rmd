...
outputs <- function(model){
  ...
  test_fit$observed = subdata$"resp"
  
  ptest = predict(fit, newdata=tvst)
  names(ptest) = rownames(subdata)
  ...
  cat("\n\n\n ### Model Evaluation (on Entire Set!)\n")
...
plot.list = list(
  ...
  bio_facet(text = "Refractory", angle=0, size=7),
    ...
    # Plots  
  bio_facet(text = "Test ROC", size=7), 
  out_rtx_bl[[1]]+ labs(title=names[out_rtx_bl$fit$method]) + theme_tr, 
  out_toc_bl[[1]]+ labs(title=names[out_toc_bl$fit$method]) + theme_tr, 
  out_any[[1]]+ labs(title=names[out_any$fit$method]) + theme_tr, 
  bio_facet(text = "Left-Out ROC", size=7), 
  out_rtx_bl[[2]]+ labs(title=NULL) + theme_tr, 
  out_toc_bl[[2]]+ labs(title=NULL) + theme_tr, 
  out_any[[2]]+ labs(title=NULL) + theme_tr, 
  bio_facet(text = "Variable Importance", size=7), 
...
cairo_pdf("/media/gcpeac/R4RA_machine_learning/Data/Initial_resp_clinical_fig.pdf", height=0.8*0.8*v*1.7*11.75, width=0.8*1.7*8.25)
p
dev.off()
